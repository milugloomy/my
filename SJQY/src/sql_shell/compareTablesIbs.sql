declare 
  ...
  procedure compareTablesIbs is
  begin
    --tables in sit,not in dev
    for curSit in (select table_name from user_tables a where not exists 
      (select * from user_tables@my_tj_dev_ibs where TABLE_NAME=a.TABLE_NAME))
    loop
      dbms_output.put_line('测试库的表'||curSit.table_name||'在开发库没有');
    end loop;
    
    --tables in dev,not in sit
    for curDev in (select table_name from user_tables@my_tj_dev_ibs a where not exists 
  ...
  procedure compareColumnsIbs is
    v_count number;
  ...
  begin
    for curT in c_table loop
      ...
      select count(*) into v_count from user_tab_cols a where a.table_name=curT.table_name and not exists 
        (select * from user_tab_cols@my_tj_dev_ibs where column_name=a.column_name and table_name=a.table_name);
      if v_count>0 then
      ...
      select count(*) into v_count from user_tab_cols@my_tj_dev_ibs a where a.table_name=curT.table_name and not exists 
        (select * from user_tab_cols where column_name=a.column_name and table_name=a.table_name);
      if v_count>0 then
        for curC in (select column_name from user_tab_cols@my_tj_dev_ibs a where a.table_name=curT.table_name and not exists 
          (select * from user_tab_cols where column_name=a.column_name and table_name=a.table_name))
        loop
      ...
      select count(*) into v_count from user_tab_cols a,user_tab_cols@my_tj_dev_ibs b 
      ...
      if v_count>0 then
        for curC in (select a.TABLE_NAME,a.COLUMN_NAME,a.DATA_TYPE SIT_DATA_TYPE,b.DATA_TYPE DEV_DATA_TYPE,
          ...
          from user_tab_cols a,user_tab_cols@my_tj_dev_ibs b where a.TABLE_NAME=b.TABLE_NAME 
...
begin
  ...
  if v_current_user <> 'IBS' then
    raise_application_error(-20001,'只允许IBS用户执行此程序');
  end if;
  compareTablesIbs;
