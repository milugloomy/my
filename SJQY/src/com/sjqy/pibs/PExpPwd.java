...
@Service("PExpPwdBatchTransAction")
public class PExpPwd  extends AbstractParentBatch{
	...
	public void execute(Map<String, Object> context) {
		...
		File file=new File(p.getProperty("pexpPath"));
		BufferedWriter bw=null;
		try {
			bw=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file)));
			Map<String,Object> paraMap=new HashMap<String,Object>();
			paraMap.put("MChannel", "PIBS");
			int total=(Integer) this.sqlMapECIF.queryForObject("ecif.queryTotalPCif",paraMap);
			int startNum=0;
			while(startNum<total){
				StringBuffer sb=new StringBuffer();
				paraMap.put("StartNum", startNum);
				paraMap.put("EndNum", startNum+number);
				List<Map> list=this.sqlMapECIF.queryForList ("ecif.querySecPcif",paraMap);
				...
				bw.flush();
				System.out.println("-----��д��"+(startNum+list.size())+"��-----");
				startNum+=number;
			}
		} catch (IOException e) {
			e.printStackTrace();
